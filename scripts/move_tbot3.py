...
parser.add_argument('-id', help='which robot to control', metavar='0', default=0, type=int)
...
class moveTbot3:
	def __init__(self, id):
		...
		self.actions = String()
		self.pose = Pose()
		self.vel_pub = rospy.Publisher(self.id+'/cmd_vel',Twist,queue_size=10)
		self.action_subscriber = rospy.Subscriber(self.id+'/actions',String,self.callback_actions)
		self.pid_subscriber = rospy.Subscriber("/Controller_Status",String,self.callback_pid)
		self.pose_subscriber = rospy.Subscriber(self.id+'/odom',Odometry,self.pose_callback)
		self.status_publisher = rospy.Publisher(self.id+"/status",String,queue_size = 10)
...
if __name__ == "__main__":
	try:
		moveTbot3(parser.parse_args().id)
